...
const register = async (req, res, next) => {
    try {
        ...
        await notificationService.notifyUser(user.email, user.firstName, 'welcome');

        // --- NEW: Notify guardian if the user is female and guardian email is provided ---
    ...
    try {
        ...
        if (paymentProcessor === "paypal") {
            try {
                ...
                await notificationService.notifyUser(user.email, user.firstName, 'subscription_failure', null, { type: 'PayPal', reason: paypalError.message });
                if (user.gender === 'female' && user.guardianEmail) {
                    await notificationService.notifyGuardian(user.guardianEmail, user.firstName, 'subscription_failure', null, { type: 'PayPal', reason: paypalError.message });
                }
                throw createError(
                    400,
        ...
        if (paymentProcessor === "stripe") {
            ...
            if (!["active", "trialing"].includes(subscription.status)) {
                // --- NEW: Notify user and guardian about failed subscription ---
                await notificationService.notifyUser(user.email, user.firstName, 'subscription_failure', null, { type: 'Stripe', reason: `Status: ${subscription.status}` });
                if (user.gender === 'female' && user.guardianEmail) {
                    await notificationService.notifyGuardian(user.guardianEmail, user.firstName, 'subscription_failure', null, { type: 'Stripe', reason: `Status: ${subscription.status}` });
                }
                throw createError(
                    400,
            ...
            await notificationService.notifyUser(user.email, user.firstName, 'subscription_success', null, { type: 'Stripe' });
            if (user.gender === 'female' && user.guardianEmail) {
                await notificationService.notifyGuardian(user.guardianEmail, user.firstName, 'subscription_success', null, { type: 'Stripe' });
            }
        ...
        if (user) { // Ensure user object is available before attempting to notify
            await notificationService.notifyUser(user.email, user.firstName, 'subscription_failure', null, { type: 'General', reason: error.message });
            if (user.gender === 'female' && user.guardianEmail) {
                await notificationService.notifyGuardian(user.guardianEmail, user.firstName, 'subscription_failure', null, { type: 'General', reason: error.message });
            }
        }
        next(error);
    }
...
const confirmPaypalSubscription = async (req, res, next) => {
    ...
    try {
        ...
        if (!["ACTIVE", "APPROVAL_PENDING"].includes(subscriptionData.status)) {
            // --- NEW: Notify user and guardian about failed subscription ---
            await notificationService.notifyUser(user.email, user.firstName, 'subscription_failure', null, { type: 'PayPal', reason: `Status: ${subscriptionData.status}` });
            if (user.gender === 'female' && user.guardianEmail) {
                await notificationService.notifyGuardian(user.guardianEmail, user.firstName, 'subscription_failure', null, { type: 'PayPal', reason: `Status: ${subscriptionData.status}` });
            }
            throw createError(
                400,
        ...
        // --- NEW: Notify user and guardian about successful subscription ---
        await notificationService.notifyUser(user.email, user.firstName, 'subscription_success', null, { type: 'PayPal' });
        if (user.gender === 'female' && user.guardianEmail) {
            await notificationService.notifyGuardian(user.guardianEmail, user.firstName, 'subscription_success', null, { type: 'PayPal' });
        }
        ...
        if (user) { // Ensure user object is available before attempting to notify
            await notificationService.notifyUser(user.email, user.firstName, 'subscription_failure', null, { type: 'PayPal Confirmation', reason: error.message });
            if (user.gender === 'female' && user.guardianEmail) {
                await notificationService.notifyGuardian(user.guardianEmail, user.firstName, 'subscription_failure', null, { type: 'PayPal Confirmation', reason: error.message });
            }
        }
        next(error);
    }
